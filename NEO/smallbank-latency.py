...
def custom_background_code():
    ...
    """
    #Check and wait for complete blockchain-database
    ...
    while True:
        if time.time()-tBegin <  300:
            if iCounter % 2 == 0:
                ...
                writeLatency = nb.invokeMethod([smart_contract_addr, 2, ["A","B",10,iNonce]])        
                iNonce = iNonce + 1
                #print writeLatency to console
                print("Latency (write):",writeLatency, "Seconds")
                #measure latency for read-operation
                readLatency = nb.invokeMethod([smart_contract_addr, 1, ["B",iNonce]])
                iNonce = iNonce + 1
                ...
                #measure latency for write-operation
                writeLatency = nb.invokeMethod([smart_contract_addr, 2, ["B","A",10,iNonce]]) 
                iNonce = iNonce + 1
                #print writeLatency to console
                print("Latency (write):",writeLatency, "Seconds")
                #measure latency for read-operation
                readLatency = nb.invokeMethod([smart_contract_addr, 1, ["A",iNonce]])
                iNonce = iNonce + 1
                #print readLatency to console
                print("Latency (read):",readLatency, "Seconds")
                #write Lantency-value into file
